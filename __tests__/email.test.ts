...
describe('The Email Class', () => {
	describe('When Connecting', () => {
		...
		it('Should call imap.connect', async () => {
			const spy = jest.spyOn(emailClass._imap, 'connect').mockImplementation(() => {
	...
	describe('getEmails', () => {
		...
		it('Should use inbox if none is specified', async () => {
			jest.spyOn(emailClass._imap, 'openBox').mockImplementation((box, _readonly, cb: any) => {
		...
		it('Should reject on openBox errors', async () => {
			jest.spyOn(emailClass._imap, 'openBox').mockImplementation((_box, _readonly, cb: any) => {
		...
		it('Should call fetchEmails', async () => {
			...
			const spy = jest.spyOn(emailClass, '_fetchEmails').mockImplementation(() => {
				return Promise.resolve(['']);
			});
			await email.getEmails();
			expect(spy).toHaveBeenCalled();
